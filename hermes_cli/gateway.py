...
def _dcp_log(msg: str) -> None:
    """Tagged logger for DCP orchestration steps. Stdout so it's visible
    next to the gateway's normal output."""
    print(f"[dcp-orchestrator] {msg}", flush=True)


def _dcp_diag_run_agent(prompt: str) -> str:
    """Run the LLM agent for DCP orchestration WITHOUT oneshot's
    stdout/stderr redirection so we can see the real error. Returns
    the response text or an empty string. Logs everything via _dcp_log
    so failures are visible."""
    os.environ["HERMES_YOLO_MODE"] = "1"
    os.environ["HERMES_ACCEPT_HOOKS"] = "1"
    try:
        from hermes_cli.oneshot import _run_agent
        _dcp_log("invoking _run_agent (no stdout redirect — verbose)")
        try:
            response = _run_agent(prompt, toolsets="all")
            if response:
                _dcp_log(f"agent response ({len(response)} chars): {response[:500]}")
            else:
                _dcp_log("agent returned empty response (no error raised)")
            return response or ""
        except Exception as exc:
            import traceback
            _dcp_log(f"_run_agent raised: {type(exc).__name__}: {exc}")
            for line in traceback.format_exc().rstrip().split("\n")[:20]:
                _dcp_log(f"  trace: {line}")
            return ""
    except Exception as exc:
        _dcp_log(f"could not import _run_agent: {exc}")
        return ""


def _dcp_load_provider_key() -> Optional[str]:
    """Read DCP_PROVIDER_KEY from ~/.dcp/env (written by agent-install.sh)."""
    env_path = os.path.expanduser("~/.dcp/env")
    ...
    try:
        with open(env_path, "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if line.startswith("DCP_PROVIDER_KEY="):
...
def _dcp_run(cmd, *, check=False, capture=True, timeout=120) -> "subprocess.CompletedProcess":
    """Run a subprocess, log it, and return the result. Non-fatal on
    error unless check=True."""
    import subprocess
    ...
    try:
        ...
        _dcp_log(f"  TIMEOUT after {timeout}s")
        raise
    except Exception as exc:
        _dcp_log(f"  EXCEPTION: {exc}")
        raise


def _dcp_step_sudoers(user: str) -> bool:
    """Write /etc/sudoers.d/dcp-agent so the agent can run wg/wg-quick/
    nvidia-smi/systemctl without prompting. Skips if the file already
    exists with our marker. Returns True on success."""
    sudoers_path = "/etc/sudoers.d/dcp-agent"
    if os.path.exists(sudoers_path):
        try:
            content = open(sudoers_path, "r", encoding="utf-8").read()
            if "dcp-agent NOPASSWD" in content or user in content:
                _dcp_log(f"sudoers already in place at {sudoers_path}")
                return True
        except PermissionError:
            # File exists but we can't read it as non-root. Trust it.
            _dcp_log(f"sudoers exists at {sudoers_path} (no read perm — assuming OK)")
            return True
    body = (
        f"# DCP Agent — passwordless sudo. Generated by hermes_cli.gateway.\n"
        f"{user} ALL=(ALL) NOPASSWD: /usr/bin/wg, /usr/bin/wg-quick, "
        f"/usr/sbin/wg, /usr/sbin/wg-quick, /usr/bin/nvidia-smi\n"
        f"{user} ALL=(ALL) NOPASSWD: /bin/systemctl, /usr/bin/systemctl\n"
        f"{user} ALL=(ALL) NOPASSWD: /usr/sbin/ip, /sbin/ip\n"
    )
    try:
        # We can't write /etc/sudoers.d directly without root. Try sudo
        # with -n (no password) first; if that fails, instruct.
        proc = _dcp_run(
            ["sudo", "-n", "tee", sudoers_path],
            capture=True, timeout=10,
        )
        if proc.returncode != 0:
            # Fall through to non-elevated attempt (e.g. running as root)
            with open(sudoers_path, "w", encoding="utf-8") as f:
                f.write(body)
            os.chmod(sudoers_path, 0o440)
            _dcp_log(f"wrote {sudoers_path} as root")
            return True
        # sudo -n worked but tee needs the body via stdin; redo properly.
        import subprocess
        sub = subprocess.run(
            ["sudo", "-n", "tee", sudoers_path],
        ...
        if sub.returncode == 0:
            subprocess.run(["sudo", "-n", "chmod", "440", sudoers_path], timeout=5)
            _dcp_log(f"wrote {sudoers_path} via sudo")
            return True
    except Exception as exc:
        _dcp_log(f"sudoers write failed: {exc}")
    _dcp_log("WARN: could not write sudoers — agent will work but will prompt for sudo")
    return False


def _dcp_step_wireguard_setup(provider_key: str, api_base: str) -> Optional[str]:
    """Generate WG keypair if missing, register with api.dcp.sa, write
    /etc/wireguard/wg0.conf, bring tunnel up. Returns the assigned
    10.8.0.X IP on success, None on failure. Idempotent — if a tunnel
    is already up, just returns its IP."""
    import subprocess
    ...
    try:
        ...
        if proc.returncode == 0 and "interface: wg0" in proc.stdout:
            # Already up. Find our IP.
            ip_proc = subprocess.run(
                ["ip", "-4", "addr", "show", "dev", "wg0"],
                capture_output=True, text=True, timeout=5,
            )
            for token in ip_proc.stdout.split():
                if token.startswith("10.8.0."):
                    ip = token.split("/")[0]
                    _dcp_log(f"WG already up at {ip}")
                    return ip
    except Exception:
        pass

    # Generate keypair if not present in ~/.dcp/wg
    home = os.path.expanduser("~")
    wg_dir = os.path.join(home, ".dcp", "wg")
    os.makedirs(wg_dir, exist_ok=True)
    priv_path = os.path.join(wg_dir, "private.key")
    pub_path = os.path.join(wg_dir, "public.key")
    if not os.path.exists(priv_path):
        _dcp_log("generating WG keypair")
        try:
            ...
            _dcp_log(f"WG keygen failed: {exc}")
            return None
    priv = open(priv_path).read().strip()
    pub = open(pub_path).read().strip()

    # Register with backend
    _dcp_log(f"registering WG pubkey {pub[:16]}... with {api_base}")
    try:
        import urllib.request
        req = urllib.request.Request(
            f"{api_base}/api/providers/wg/register",
            data=_json.dumps({"public_key": pub}).encode("utf-8"),
            headers={
        ...
        _dcp_log(f"WG register HTTP failed: {exc}")
        return None
    ip = resp.get("ip")
    server_pub = resp.get("server_pubkey")
    server_endpoint = resp.get("server_endpoint")
    psk = resp.get("psk", "")
    if not ip or not server_pub or not server_endpoint:
        _dcp_log(f"WG register response missing fields: {resp}")
        return None
    _dcp_log(f"assigned IP={ip}, server={server_endpoint}")

    # Write wg0.conf
    conf = (
        f"[Interface]\nPrivateKey = {priv}\nAddress = {ip}/24\n\n"
        f"[Peer]\nPublicKey = {server_pub}\n"
        f"{'PresharedKey = ' + psk + chr(10) if psk else ''}"
    ...
    try:
        ...
        if sub.returncode != 0:
            _dcp_log(f"could not write {conf_path}: {sub.stderr}")
            return None
        subprocess.run(["sudo", "-n", "chmod", "600", conf_path], timeout=5)
    except Exception as exc:
        _dcp_log(f"wg0.conf write failed: {exc}")
        return None

    # Bring up
    _dcp_log("bringing up wg0")
    try:
        # Try to bring down first in case stale
        subprocess.run(["sudo", "-n", "wg-quick", "down", "wg0"],
                       capture_output=True, timeout=10)
        up = subprocess.run(["sudo", "-n", "wg-quick", "up", "wg0"],
                            capture_output=True, text=True, timeout=20)
        if up.returncode != 0:
            _dcp_log(f"wg-quick up failed: {up.stderr}")
            return None
    except Exception as exc:
        _dcp_log(f"wg-quick exception: {exc}")
        return None
    _dcp_log(f"WG tunnel up at {ip}")
    return ip


def _dcp_step_detect_engine_endpoint(wg_ip: str) -> Optional[str]:
    """Pick whichever inference engine is actually serving on this box
    and return the URL the renter will hit (over WG). Prefers vLLM
    (port 8000) over Ollama (11434). Returns None if neither found."""
    import socket
    for port, name in [(8000, "vllm"), (11434, "ollama")]:
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.settimeout(2)
            s.connect(("127.0.0.1", port))
            s.close()
            _dcp_log(f"detected {name} on local 127.0.0.1:{port}")
            return f"http://{wg_ip}:{port}"
        except Exception:
            continue
    _dcp_log("no inference engine detected on 8000 or 11434")
    return None


def _dcp_step_register_endpoint(provider_key: str, api_base: str,
                                 endpoint_url: str) -> bool:
    """POST our vllm_endpoint_url to the backend so renter requests
    route to us. The backend has a generic provider update endpoint
    plus the heartbeat path can carry endpoint info."""
    import urllib.request
    import json as _json
    try:
        req = urllib.request.Request(
            f"{api_base}/api/providers/heartbeat",
            data=_json.dumps({
                "vllm_endpoint_url": endpoint_url,
                "wg_mesh_ip": endpoint_url.split("://")[1].split(":")[0],
            }).encode("utf-8"),
            headers={
                "Content-Type": "application/json",
                "x-provider-key": provider_key,
        ...
        with urllib.request.urlopen(req, timeout=10) as r:
            r.read()
        _dcp_log(f"endpoint registered: {endpoint_url}")
        return True
    except Exception as exc:
        _dcp_log(f"endpoint register failed: {exc}")
        return False


def _dcp_heartbeat_loop(provider_key: str, api_base: str,
                        endpoint_url: Optional[str], stop_event) -> None:
    """Background thread — POSTs /api/providers/heartbeat every 30s so
    the backend's providerHealthWorker keeps us as status='online'."""
    import time as _time
    import urllib.request
    import json as _json
    interval = 30
    ...
    while not stop_event.is_set():
        try:
            # Backend wants the provider api_key in the JSON body, not
            # the x-provider-key header (verified by agent's self-debug
            # during 2026-05-10 first-run-orchestration test). Sending
            # both for forward-compat in case backend later migrates.
            payload = {"api_key": provider_key, "gpu_status": "ready"}
            if endpoint_url:
                payload["vllm_endpoint_url"] = endpoint_url
                wg_ip = endpoint_url.split("://")[1].split(":")[0]
            ...
            with urllib.request.urlopen(req, timeout=10) as r:
                r.read()
            if fail_count > 0:
                _dcp_log(f"heartbeat recovered after {fail_count} failures")
                fail_count = 0
        except Exception as exc:
            fail_count += 1
            if fail_count <= 3 or fail_count % 10 == 0:
                _dcp_log(f"heartbeat failed (#{fail_count}): {exc}")
        # Sleep in 1s slices so stop_event interrupts cleanly
...
def _run_dcp_first_run_orchestration_if_needed() -> None:
    """First-run setup for a freshly-installed DCP provider box.

    Hands the work to the LLM agent (Hermes via api.dcp.sa gateway) —
    if the agent can't drive the install, we WANT to see that fail
    loudly here, because that's the actual product gap. No Python
    pipeline pretending to be the agent.

    Trigger: ``~/.dcp/env`` (with DCP_PROVIDER_KEY) exists. Marker
    ``~/.dcp/agent-initialized`` signals first-run already ran cleanly.

    The diagnostic helper ``_dcp_diag_run_agent`` calls _run_agent
    WITHOUT oneshot's stderr/stdout redirect so any failure is visible
    in the gateway log."""
    home = os.path.expanduser("~")
    dcp_dir = os.path.join(home, ".dcp")
    init_marker = os.path.join(dcp_dir, "agent-initialized")

    provider_key = _dcp_load_provider_key()
    if not provider_key:
        return  # not a DCP install (developer-mode run from a clone)

    api_base = os.environ.get("DCP_API_BASE", "https://api.dcp.sa").rstrip("/")

    # Always start the heartbeat thread when a provider_key exists, even
    # on subsequent boots that skip orchestration. Without continuous
    # heartbeats, providerHealthWorker demotes to status=disconnected
    # within ~60s. Daemon thread runs for the lifetime of the gateway.
    import threading
    _stop_event = threading.Event()
    # Read previously-set endpoint from the marker if available so the
    # thread can re-assert it on every heartbeat (idempotent + survives
    # backend resets). Endpoint shape: http://10.8.0.X:PORT.
    ...
    if os.path.exists(init_marker):
        try:
            ...
            for line in content.splitlines():
                if "http://10.8.0." in line:
                    # Crude extraction — the marker may include the URL
                    # in a phrase like "WG tunnel up at 10.8.0.X" or
                    # similar. Rebuild from the IP + port 11434 (Ollama
                    # default) as a fallback if the agent didn't write
                    # the explicit URL.
                    pass
        except Exception:
            pass
    _hb_thread = threading.Thread(
        target=_dcp_heartbeat_loop,
        args=(provider_key, api_base, _endpoint_url, _stop_event),
        name="dcp-heartbeat",
        daemon=True,
    )
    _hb_thread.start()
    _dcp_log("heartbeat thread started (30s interval)")
    globals()["_dcp_heartbeat_stop_event"] = _stop_event

    if os.path.exists(init_marker):
        return  # already orchestrated; subsequent boots skip the prompt

    print("┌─────────────────────────────────────────────────────────┐")
    print("│  DCP first-run orchestration via Hermes agent           │")
    print("└─────────────────────────────────────────────────────────┘", flush=True)

    user = os.environ.get("USER") or os.environ.get("LOGNAME") or "ubuntu"

    prompt = f"""You are doing first-run setup for a fresh DCP provider machine. Execute these steps in order using your shell tool. Do NOT ask the user — they explicitly want you to do it all autonomously.

The provider's DCP_PROVIDER_KEY is in ~/.dcp/env. The current user is `{user}`. The DCP API base is {api_base}.

Step 1 — passwordless sudo for WG/GPU commands:
  Use the shell tool with sudo to write /etc/sudoers.d/dcp-agent containing:
    {user} ALL=(ALL) NOPASSWD: /usr/bin/wg, /usr/bin/wg-quick, /usr/sbin/wg, /usr/sbin/wg-quick, /usr/bin/nvidia-smi
    {user} ALL=(ALL) NOPASSWD: /bin/systemctl, /usr/bin/systemctl, /usr/sbin/ip, /sbin/ip
  Then: sudo chmod 440 /etc/sudoers.d/dcp-agent

Step 2 — WireGuard registration with the backend:
  Generate a WG keypair at ~/.dcp/wg/private.key + public.key (mode 600 on private).
  POST the public_key to {api_base}/api/providers/wg/register with header `x-provider-key: <DCP_PROVIDER_KEY from ~/.dcp/env>` and body {{"public_key": "<pub>"}}.
  Parse the JSON response: keys are `ip`, `server_pubkey`, `server_endpoint`, `psk` (optional).
  Write /etc/wireguard/wg0.conf:
    ...
    PresharedKey = <psk>  (only if psk present)
    Endpoint = <server_endpoint>
    AllowedIPs = 10.8.0.0/24
    PersistentKeepalive = 25
  Run: sudo wg-quick up wg0

Step 3 — detect or install the local inference engine, then register the endpoint:
  3a. Check whether port 8000 (vLLM) or 11434 (Ollama) is listening on 127.0.0.1 via `ss -tlnp | grep -E ':8000|:11434'` or `curl -s localhost:8000/v1/models` / `curl -s localhost:11434/api/tags`.
  3b. If NEITHER is listening, install Ollama and pull a small model so we have something to serve. The provider will not be useful otherwise:
      - Run: `curl -fsSL https://ollama.com/install.sh | sh` (this auto-installs ollama as a systemd service and starts it).
      - The default Ollama systemd binds to 127.0.0.1, but our backend probe needs to reach it over WG. Override by writing /etc/systemd/system/ollama.service.d/override.conf with `[Service]\\nEnvironment="OLLAMA_HOST=0.0.0.0:11434"` then run `sudo systemctl daemon-reload && sudo systemctl restart ollama`. If systemd isn't available (e.g. minimal container), kill the default ollama process and start one with `OLLAMA_HOST=0.0.0.0:11434 nohup ollama serve > /tmp/ollama.log 2>&1 &`.
      - Wait for the API to come up (poll `curl -s localhost:11434/api/tags` until 200, max 30s).
      - Pull a small starter model: `ollama pull qwen3:0.6b` (~500MB, fast to demo). This proves the box can serve. We can swap to a bigger model later via dcp-model-auto-select.
  3c. Verify the chosen port (11434 if you installed Ollama, 8000 if vLLM was already there) is now reachable on 127.0.0.1, and POST to {api_base}/api/providers/heartbeat with header `x-provider-key: ...` and body {{"vllm_endpoint_url": "http://<wg_ip>:<port>", "wg_mesh_ip": "<wg_ip>"}}. The endpoint URL uses the WG IP (not localhost) so the backend's reachability probe over the WG mesh succeeds.

Step 4 — touch ~/.dcp/agent-initialized with a one-line summary of what you did.

Report ONE concise line per step as you go. If any step fails, report which one and why and STOP — do NOT touch the marker. You have full shell access (HERMES_YOLO_MODE=1) so just run the commands."""

    response = _dcp_diag_run_agent(prompt)
    if not response:
        _dcp_log("agent returned empty — orchestration did not run.")
        _dcp_log("This is the real gap: Hermes/MiniMax brain isn't producing output.")
        _dcp_log("Gateway will start anyway; provider will be invisible to backend.")
        _dcp_log("FIX: debug oneshot's _run_agent — likely runtime_provider auth or base_url.")
        return

    # If the agent reported success and touched the marker, we're done.
    if os.path.exists(init_marker):
        _dcp_log("first-run orchestration complete — marker written by agent.")
    else:
        _dcp_log("agent reported but no marker — partial success, will retry on next boot.")
